...
| `[publish]` | 省略可能なテーブル。ビルドの起点となる tree。[vault の一部を公開する](#vault-の一部を公開する)を参照 |
...
vault はサイトではありません。書いたもののほとんどは公開するためのものではなく、
公開／非公開はファイルを移動することで区別しているのが普通です。

```toml
[publish]
# パスはソースルートからの相対。ここで名指した tree と、そこから辿れる tree
# だけがコンパイルされる。
from = ["MIYA-LIS.NET/**"]
```

`**` はディレクトリ区切りを跨ぎ、`*` と `?` は跨ぎません。

名指した tree から辿れるものは推移的にすべて引き込まれます。wiki リンク、埋め込み、
`authors:` の帰属、そして解決した Markdown リンクを通じてです。したがって公開ページ
は vault のどこに置いたノートを参照してもよく、そのノートは参照されているがゆえに
公開されます。

```text
MIYA-LIS.NET/index.tree.md    パターンが名指している
MIYA-LIS.NET/notes.tree.md    パターンが名指している
notes/frege.tree.md           notes の ![[frege.tree]] から辿れる
DAILY/2026-08-19.tree.md      どちらでもない: コンパイルされない
```

**公開されないノートはそもそもコンパイルされません** — 出力もされず、診断も出ません。
下書きの壊れたリンクが、それを載せていないページのビルドを失敗させてはいけないから
です。件数だけは summary に出るので、公開するつもりでリンクし忘れたノートが黙って
消えることはありません。
...
どちらのパスが先に並ぶかで黙って決めるのではなく `TM201` として報告します。

発番も公開に従います。このビルドが載せない tree は、このビルドが書き換えるもの
ではありません。

このテーブルが無ければソースルート配下のすべてが公開されます。テーブルが存在する
前の forest の挙動そのままです。
