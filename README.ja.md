...
Forester では `[label](addr)` は URL ではなく **tree 参照**です。相対リンクの
形式は存在しません。したがって外部 URI でない宛先を持つ Markdown リンクは
tree を指しており、wiki リンクとまったく同じ規則で解決されます。mdbase v0.3
§08 も Markdown リンクをリンクとして数えています。
...
解決できないローカル宛先は `TM202` です。以前のように素通しにすると、どの tree
も持たないアドレスへの参照が出力に入り、しかも何も報告されませんでした。
