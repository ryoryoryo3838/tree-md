...
| 出力パス | ディレクトリ構造を反映。`trees-md/a/foo.tree.md` → `generated/a/foo.tree` |
| tree の同一性 | **ファイル名の語幹**（`foo`）であり、パス（`a/foo`）ではない。forest 内で語幹が重複するとエラー |
...
| `##` – `######` | 入れ子の `\subtree{ \title{...} ... }` |
| 見出しの直前の `<!-- subtree: ID -->` | 名前付き `\subtree[ID]{ ... }` |
...
無名の `\subtree` はアドレス可能な名前を持ちません。節へリンクするには、
...
文法は厳密に `<!-- subtree: ID -->`（ASCII 空白）です。ID は
`[A-Za-z0-9][A-Za-z0-9._-]*` に一致します。ディレクティブの直後は必ず見出しで
なければならず、孤立・重複・不正なディレクティブはエラーです。
